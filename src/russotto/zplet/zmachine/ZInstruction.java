...
public class ZInstruction {
	...
	public ZInstruction(int opnum, int count, short [] operands, short storevar, short branchoffset, boolean branchtype) {
				this.opnum = opnum;
				this.count = count;
	...
	protected void decode_first_half() {
				...
				case 0xC0: /* variable form */
					...
					if ((opcode & 0x20) == 0) { /* 2OP */
								/* note that variable-form je can have more than
								   two arguments, though it is a 2OP */
								opnum = opcode & 0x1F;
					}
					count = 0;
	...
	public void execute()
	{
				...
				if (isbranch()) {
					if ((result == 0) != branchtype) {
								/* that is, if result is 0 and branchtype is false, or
								   result is nonzero and branchtype is true */
								switch (branchoffset) {
								case 0:
									z_ret();
...
//			zm.print_ascii_string("\n");
	...
	{
				int byte_index = (operands[0]&0xFFFF) + 
					((operands[1]&0xFFFF) * 2);
				short result;
...
//								branchoffset = (short)(((branchoffset&0x3F) << 8) | (((short)zm.get_code_byte())&0xFF));
	...
	protected short op_call()
	{
				...
				if (operands[0] == 0) {
					...
					/* Can't just return false because call is not treated as a 
					   store in execute_instruction */
					zm.set_variable(storevar, ZFALSE);
				}
				else {
...
//					System.err.println(Integer.toString(zm.pc, 16));
					...
					for (i = 0; i < nlocals; i++) {
								thislocal = (short)(((zm.get_code_byte()<<8) & 0xFF00) |
...
//				System.err.println("textbuf = " + Integer.toString(tbuf, 16));
				...
				bufloc = 1;
				while ((tsize != 0) &&
					   ((ch = zm.get_input_byte(true)) != 13) &&
					   (ch != 10)) {
//					System.err.println("sr_ch @ " + Integer.toString(tbuf+bufloc, 16) + " : " + ch);
